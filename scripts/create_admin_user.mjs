...
async function ensureUserAndSendInvite(email) {
  // Tente d'envoyer une invitation (crée l'utilisateur s'il n'existe pas et envoie un email)
  const { data, error } = await supabase.auth.admin.inviteUserByEmail(email, { redirectTo: REDIRECT_TO });
  if (!error && data?.user?.id) {
    return data.user;
  }
  // Fallback: si l'invite échoue car l'utilisateur existe déjà, le récupérer
...
async function main() {
  try {
    ...
    const user = await ensureUserAndSendInvite(EMAIL);
    await setTempPasswordAndMetadata(user.id, tempPassword);
    await promoteToAdmin(user.id);

    // Stockage local sécurisé (ne pas committer). Crée un dossier secrets/ ignoré par git.
