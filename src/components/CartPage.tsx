...
import { SimulatedPaymentModal, PaymentMethod } from "./SimulatedPaymentModal";
...
function OrderSummary() {
  ...
  const handleConfirmPayment = (method: PaymentMethod) => {
    ...
    try {
      ...
      localStorage.setItem("user_orders", JSON.stringify(existing));
    } catch (_) {
      // ignore localStorage issues
    }

    toast.success(`Paiement simulé (${method}) réussi — aucune transaction réelle.`);
    clearCart();
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 sticky top-24">
      ...
      <div className="space-y-3">
        <button onClick={() => setOpenPayment(true)} className="w-full py-4 bg-gradient-to-r from-emerald-600 to-green-600 text-white font-['Inter',_sans-serif] font-semibold rounded-xl hover:from-emerald-700 hover:to-green-700 transition-all duration-300 shadow-lg hover:shadow-xl text-center">
          Passer la commande
        </button>
